...
int32_t danpZmqTx(danpInterface_t* iface, danpPacket_t* packet) {
    ...
    danpLogMessage(DANP_LOG_VERBOSE, "ZMQ TX: dst=%u port=%u flags=0x%02X len=%u", dst, dPort, flags, packet->length);
...
void* zmqRxRoutine(void* arg) {
    while (1) {
        ...
        if (len >= 4) {
            ...
            danpLogMessage(DANP_LOG_VERBOSE, "ZMQ RX: dst=%u port=%u flags=0x%02X len=%d", dst, dPort, flags, len-4);
...
void danpZmqInit(const char* pubBindEndpoint, const char** subConnectEndpoints, int32_t subCount, uint16_t nodeId) {
